    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        Parse.initialize(
            with: ParseClientConfiguration(block: { (configuration: ParseMutableClientConfiguration) -> Void in
                ...
                configuration.clientKey = "thisismymasterkey"  // set to nil assuming you have not set clientKey
                configuration.server = "https://aqueous-cliffs-83163.herokuapp.com/parse"
            })
        )
        
        //code to check if user is logged in already(persistent login)
